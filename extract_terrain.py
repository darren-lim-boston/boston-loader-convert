...
def simplify_scene():
    # simplify the mesh; delete all objects that are not terrain
        ...
        for i in range(size):
            ...
            if not line.startswith("map_Kd"):
                ...
                if line.startswith("newmtl ") and line.endswith("jpg"):
                    last_newmtl = line
    with open("../tiles_terrain/" + name + ".mtl", "w") as file:
        file.write("".join(new_lines))

if __name__ == "__main__":
    letter = sys.argv[-1]
    
    name = "combined_" + letter
    
    newName = "terrain_only_" + letter
    
    pre_simplify_scene()    
    import_obj("../tiles_obj_merged/" + name + ".obj")
    simplify_scene()
    export_obj("../tiles_terrain/" + newName + ".obj", newName)